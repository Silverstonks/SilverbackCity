	...
	return (
		<div className="container">
			<div className="main-home-slider slider" id="main-home-slider">
				...
				>
					<div className="slider-content mt-5 only-desktop">
						<img src={DotsDecoration} alt="" className="img-detail dots" />

						{
							navPills.find((navPill, index) => index + 1 === selectedIndex)
								.content
						}
					</div>
          ...
          where the map only reads what is exactly in the translation.json file --JD*/}
					...
					<div className="slider-nav-pills-container row only-desktop">
						{navPills.map(
							(navPill, index) =>
								index < 3 && (
									<SliderItem
					...
					<div className="slider-nav-pills-container row only-mobile">
						{navPills.map((navPill, index) => (
							<div className="content">
								<div key={index}>{navPill.content}</div>
								<div>
									<h4>{t(`SLIDE_${index + 1}.title`)}</h4>
								</div>
								<div>
									<h5>{t(`SLIDE_${index + 1}.description`)}</h5>
								</div>
							</div>
						))}
					</div>
