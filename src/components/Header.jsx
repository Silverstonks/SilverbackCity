...
const Header = () => {
  ...
  window?.ethereum?.on("accountsChanged", (accounts) => {
    dispatch({
      ...
      payload: { walletAddress: accounts[0], isConnected: true }
    });
  });

  window?.ethereum?.on("networkChanged", function (networkId) {
    dispatch(actions.blockChainActions.networkChanged);
  });
              ...
              <ul className="nav" id="nav">
                ...
                <li className="nav-item dropdown">
                  ...
                  >
                    ...
                    >
                      {t("nav.third_option.dropdown.propertyHolders")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("lounge", 100)}
                    >
                      {t("nav.third_option.dropdown.lounge")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("bidsarena", 100)}
                    >
                      {t("nav.third_option.dropdown.arena")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("exchange", 100)}
                    >
                      {t("nav.third_option.dropdown.exchange")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("nft-lab", 100)}
                    >
                      {t("nav.third_option.dropdown.nftlab")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("silvershop", 100)}
                    >
                      {t("nav.third_option.dropdown.ecommerce")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("vault", 100)}
                    >
                      {t("nav.third_option.dropdown.vault")}
                    </div>
                    <div
                      className="dropdown-item nav-link p-2 text-capitalize"
                      onClick={() => scrollTo("staking", 100)}
                    >
                      {t("nav.third_option.dropdown.staking")}
                    </div>
                  </div>
                </li>
                <li
                ...
                >
                  <div className="nav-link">{t("nav.fourth_option.item")}</div>
                </li>
