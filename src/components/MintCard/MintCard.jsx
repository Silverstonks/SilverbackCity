      ...
      <div className="mint-card row">
        <div className="col-12 col-md-6 mint-card-details">
          ...
          <div className="mint-card-price-container presale">
            <span className="mint-card-price-label ">{t('publicSale')}</span><br />
            <span className="mint-card-price font-weight-bold">{t('launchPrice')}</span>
            &nbsp;
            <span>{t('coin')}</span>
          </div><br />
          <div className="mint-card-price-container text-white">
            <span className="mint-card-price-label color-white">{t('soFar')}</span>
            &nbsp;
            <span className="mint-card-price text-white">{t('soFarPrice')}</span>
          </div><br />
            <div className="mint-card-counter">
              <button onClick={()=>setCounter(counter-1)}>-</button>
              <input type="number" className='input-form' value={counter>0?counter:""} placeholder={t("howManyNFTs")} onChange={(e)=>onChange(e)}/><br/>
              <button onClick={()=>setCounter(counter+1)}>+</button>
              </div>
              <br /> 
            <div className="mint-card-buttons">
            <div className="button-container">
           <button onClick={() => alert(counter)}>{t("connectWallet")}</button>
           </div>
          <div className="button-container">
